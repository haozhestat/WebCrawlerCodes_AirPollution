...
readdir_0<-"C:/Users/haozhe/Dropbox/projects/pm2.5/data/data_0225/pm_0225/"            # location where pm data saved
weather_read<-"C:/Users/haozhe/Dropbox/projects/pm2.5/data/data_0225/weather_0225/"    # location where weather data saved
writedir<-paste0("C:/Users/haozhe/Dropbox/projects/pm2.5/data/data_0225/organized")   # location where to save the final data
  ...
  for(j in 1:length(cites))
  {
    ...
    if(dim(station_cites)[1]!=0)
    {
      ...
      for(k in 1:dim(station_cites)[1])
      {
        ...
        tempname<-paste0(readdir,"/",station_cites$province_name[i],"_",station_cites$city_name.English[k],"_",station_cites$station_code[k],".csv")
        tempdata<-read.csv(tempname)
        tempdata$pm2_5[tempdata$pm2_5==0]<-NA
        Data_temp<-cbind(Data_temp,tempdata$pm2_5)
        }
      }
      
      # Add the year month day hour  
      Data_temp<-cbind(Data_temp, year(ymd(substring(as.character(tempdata$time),0,10))))  
      Data_temp<-cbind(Data_temp, month(ymd(substring(as.character(tempdata$time),0,10))))  
      Data_temp<-cbind(Data_temp, day(ymd(substring(as.character(tempdata$time),0,10))))
      Data_temp<-cbind(Data_temp, hour(hms(substring(as.character(tempdata$time),12,19))))
      # Calculate the average pm2.5 for each city
      Data_temp<-cbind(Data_temp,rowMeans(Data_temp[,1:(dim(Data_temp)[2]-4)],na.rm = TRUE))
